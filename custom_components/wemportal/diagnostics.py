...
#
# CONF_EXPERT_MODULE_LIST is the cache of what a discovery run found, and
# every entry carries an entityvalue under the key "value" - far too generic
# a name to redact by itself, since every reading has one. The whole cache
# goes instead: it is a lookup table for the options form, not something a
# bug report needs.
TO_REDACT = {
    CONF_USERNAME,
    CONF_PASSWORD,
...
def _aliased_row_key(key: str, aliases: dict[Any, str]) -> str:
    """One reading's key with every device id in it replaced by its alias.

    Row keys are built from the device id (`1234-ConnectionStatus`,
    `1234-Energy_1`), so aliasing only the OUTER key left the id in plain
    sight one level down - in a file written to be attached to a public
    issue. Longest id first, so one id that is a prefix of another cannot
    swallow it.
    """
    aliased = key
    for device_id in sorted(aliases, key=lambda value: len(str(value)), reverse=True):
        aliased = aliased.replace(str(device_id), aliases[device_id])
    return aliased
...
async def async_get_config_entry_diagnostics(
    ...
    readings = {
        aliases[device_id]: async_redact_data(
            {
                _aliased_row_key(key, aliases): (
                    row.as_dict() if isinstance(row, Reading) else row
                )
